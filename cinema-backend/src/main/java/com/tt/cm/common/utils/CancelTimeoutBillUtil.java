package com.tt.cm.common.utils;
...
import com.tt.cm.service.impl.TicketServiceImpl;
import com.tt.cm.service.impl.SessionServiceImpl;
...
public class CancelTimeoutBillUtil {
    ...
    @Resource
    private TicketServiceImpl sysBillService;

    @Resource
    private SessionServiceImpl sysSessionService;
    ...
    public void cancelTimeoutBill() {
        // 查询所有未支付且超时的订单
        List<Ticket> timeoutBillList = sysBillService.findTimeoutBill();
        // Lamda循环遍历、处理超时订单
        timeoutBillList.forEach((timeoutBill) -> {
            ...
            Session curSession = timeoutBill.getSession();
            if (curSession == null) {
